...
const exchangeAddress = {
    'mainnet':'0x6C5d62c394358734BF79607ccAB489483533A62A',
    'ropsten':'0x6e0c7d744c0316ce943fc3495aB2d0672010B49A',
}[ethnet];
...
const exchange_abi = [{"constant":true,"inputs":[],"name":"rate","outputs":[{"name":"","type":"uint256"}],"payable":false,"stateMutability":"view","type":"function"},{"constant":true,"inputs":[],"name":"owner","outputs":[{"name":"","type":"address"}],"payable":false,"stateMutability":"view","type":"function"},{"constant":false,"inputs":[{"name":"newOwner","type":"address"}],"name":"transferOwnership","outputs":[],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":true,"inputs":[],"name":"token","outputs":[{"name":"","type":"address"}],"payable":false,"stateMutability":"view","type":"function"},{"inputs":[{"name":"_token","type":"address"},{"name":"_rate","type":"uint256"}],"payable":false,"stateMutability":"nonpayable","type":"constructor"},{"anonymous":false,"inputs":[{"indexed":true,"name":"previousOwner","type":"address"},{"indexed":true,"name":"newOwner","type":"address"}],"name":"OwnershipTransferred","type":"event"},{"constant":false,"inputs":[{"name":"_token","type":"address"}],"name":"setToken","outputs":[],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"_rate","type":"uint256"}],"name":"setRate","outputs":[],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[],"name":"buy","outputs":[],"payable":true,"stateMutability":"payable","type":"function"},{"constant":false,"inputs":[{"name":"target","type":"address"},{"name":"_data","type":"bytes"}],"name":"buy","outputs":[],"payable":true,"stateMutability":"payable","type":"function"},{"constant":false,"inputs":[],"name":"claim","outputs":[],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[],"name":"claimTokens","outputs":[],"payable":false,"stateMutability":"nonpayable","type":"function"}];
var web3js;
...
window.addEventListener('load', async function() {
    ...
    async function updateBtcPrice() {
        ...
        if (btc_reward > 0 && btc_difficulty > 0) {
            if ($('#paymentMethod').text() == 'VIP') {
                $('#btc_price').val(btc_reward / btc_difficulty);
            } else
            if ($('#paymentMethod').text() == 'ETH') {
                $('#btc_price').val(btc_reward / btc_difficulty * exchangeRate);
            } else {
                $('#btc_price').val('');
            }
        } else {
    ...
    async function updateExchange() {
        $('#ex_eth').val();
        let vipAmount = $("#ex_vip").val();
        exchangeRate = await exchange.methods.rate().call();
        $('#ex_eth').val(vipAmount / rate);
    }

    async function makeExchange() {
        const vips = $('#ex_vip').val();
        const eths = $('#ex_eth').val();
        if (!vips || vips == 0) {
            alert('Enter correct amount of VIP coins to buy');
            return;
        }
        const tokenBalance = (await token.methods.balanceOf(exchange.options.address).call()) / 10**18;
        if (tokenBalance < vips) {
            alert(`Sorry there are no ${vips} VIP coins on the sale, use any exchange.`);
            return;
        }

        if (account) {
            exchange.methods.buy().send({from: account, value: eths * 10**18})
            ...
            const estimateGas = await exchange.methods.buy().estimateGas({value: eths * 10**18, from: '0x3550c1727fa664a94089140f32094de64c0e9967'})
            $('#tx_to').val(exchange.options.address);
            $('#tx_value').val(eths);
    ...
    async function createTask() {
        ...
        if (vipBalance && btc_reward > vipBalance && $('#paymentMethod').text() != 'ETH') {
        ...
        if (tasks && tasks.length > 4 && difficulty) {
            ...
            if (price && price < tasks[Math.trunc(tasks.length/2)].price / 10**9) {
                if (!confirm(`Your price ${Math.round(price*1000000)/1000000} VIP/GH is in lowest half of tasks list, are you sure?`)) {
                    return;
                }
            }
        }
        ...
        if ($('#paymentMethod').text() == 'ETH') {
            let data = contract.methods.createBitcoinAddressPrefixTask(btc_prefix, 0, requestPublicXPoint, requestPublicYPoint).encodeABI();
            
            if (account) {
                exchange.methods.buy(contract.options.address, data).send({from: account, value: btc_reward})
                    .on('transactionHash', function(hash) {
                        let url = explorerUrl + 'tx/' + hash;
                        console.log(`Sending transaction ${url}`);
                        window.open(url, '_blank').focus();
    ...
    $('#ex_vip').bind('input', updateExchange);
    ...
    $('.dropdown-menu a').click(function() {
        ...
        if ($('#paymentMethod').text() == 'ETH') {
            $('#exchangeWarning').text('VIP coins will be bought automatically with price: ' + Math.round(1/exchangeRate*1000000)/1000000 + ' ETH');
        } else {
            $('#exchangeWarning').text('');
        }
    });
    ...
    console.log('VIP token at: ' + token.options.address);
    ...
    exchangeRate = await exchange.methods.rate().call();
    console.log('Exchange: ' + exchangeAddress + ' rate: ' + exchangeRate + ' coins: ' + Math.round(vipCoinsOnExchange/10**18*1000)/1000);
    if (vipCoinsOnExchange/10**18 > 100) {
        $('.exchangeClass').show();
    }
